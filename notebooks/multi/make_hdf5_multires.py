...
outputpath = "/data/home/ltran/masks4-test"
...
files=glob.glob(f'{datapath}/*mask.png')
...
for phase in phases: #now for each of the phases, we'll loop through the files
    ...
    hdf5_file = tables.open_file(f"{outputpath}/{dataname}-{phase}.pytable", mode='w') #open the respective pytable
...
db = tables.open_file(f"{outputpath}/{dataname}-train.pytable")
