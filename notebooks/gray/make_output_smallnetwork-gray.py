...
checkpoint = torch.load("./trg_doublesimple_best_model.pth", map_location = device)
...
arr_out = np.zeros((npatches,6,patch_size,patch_size))
# -
...
for i, (r,c) in tqdm(enumerate(centers)):
    ...
    arr_out[i,:,:,:] = np.concatenate((img_transform(image=patch)["image"], img_transform(image=patch2)["image"]), 0)
